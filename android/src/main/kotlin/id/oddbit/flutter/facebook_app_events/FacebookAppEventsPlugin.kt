...
class FacebookAppEventsPlugin : FlutterPlugin, MethodCallHandler {
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        when (call.method) {
            ...
            "setLogLevel" -> handleLogLevel(call, result)

            else -> result.notImplemented()
        }
    }

    private fun handleLogLevel(call: MethodCall, result: Result) {
        FacebookSdk.clearLoggingBehaviors()

        for (arg in call.arguments as List<*>) {
           try {
               FacebookSdk.addLoggingBehavior(LoggingBehavior.valueOf(arg.toString()))
           } finally {
               // no-op
           }
        }
